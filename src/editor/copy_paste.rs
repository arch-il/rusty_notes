...
impl Text {
    pub fn copy(&mut self) {
        if let Some(selection_start) = self.selection_start {
            ...
            if start.0 == end.0 {
                self.copy_buffer =
                    Some(vec![String::from(&self.lines[start.0][start.1..end.1 + 1])]);
            } else {
            ...
            self.copy_buffer = Some(vec![self.lines[self.cursor.0].clone(), String::new()]);
        }
    }
    ...
    pub fn paste(&mut self) {
        ...
        if let Some(copy_buffer) = &self.copy_buffer {
            if copy_buffer.len() == 1 {
                ...
                self.lines[self.cursor.0] = String::from(&line[0..self.cursor.1]) + &copy_buffer[0];
